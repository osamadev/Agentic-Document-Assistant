...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        """
        Safely evaluate mathematical expressions. Only allows basic math operations for security.
        
        Args:
            expression: The mathematical expression to evaluate (e.g., "2 + 3 * 4", "100 / 5")
        ...
        try:
            ...
            dangerous_patterns = [
                '__', 'import', 'exec', 'eval', 'open', 'file', 'system', 'subprocess',
                'os.', 'sys.', 'globals', 'locals', 'vars', 'dir', 'help'
            ]
            ...
            logger.log_tool_use(
                ...
                {"result": result, "formatted_result": formatted_result}
            )
            
            return response
            
        except ZeroDivisionError:
            error_msg = f"Error: Division by zero in expression '{expression}'"
            logger.log_tool_use(
                "calculator",
                {"expression": expression},
